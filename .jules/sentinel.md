...
## 2024-05-27 - [Timing Attack Vulnerability in Admin Token Authentication]
**Vulnerability:** A timing attack vulnerability existed in the token authentication middlewares (`worker/src/routes/admin.ts`, `worker/src/routes/ingest.ts`, `worker/src/routes/survey.ts`). The endpoints compared the user-provided `x-admin-token` to the server's `ADMIN_SECRET_TOKEN` using standard equality operator (`!==`). This fast-fails on the first non-matching character, potentially allowing an attacker to brute-force the token by analyzing the response times.
**Learning:** String comparisons of secrets should use constant-time algorithms rather than standard equality operators.
**Prevention:** Implement a constant-time comparison function (e.g., using `crypto.timingSafeEqual` in Node.js or a custom bitwise comparison function in environments where it's unavailable, like Cloudflare Workers) and use it for all token validations.
