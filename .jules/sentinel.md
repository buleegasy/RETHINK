...
## 2026-08-23 - Prevent Timing Attacks in Admin Token Authentication
**Vulnerability:** Admin authentication endpoints (`/api/admin`, `/api/knowledge`, `/api/survey/results`) used strict equality (`===` or `!==`) to compare the user-provided `x-admin-token` header against the `ADMIN_SECRET_TOKEN` environment variable.
**Learning:** Direct string comparisons in Node.js/Cloudflare Workers V8 environments short-circuit, causing the function to return as soon as it encounters the first mismatched character. This leaks timing information, potentially allowing an attacker to deduce the secret token character by character via timing attacks.
**Prevention:** Always use a constant-time comparison function for validating secrets, passwords, or HMAC signatures. In Cloudflare Workers where a native synchronous `timingSafeEqual` may not be available, a custom bitwise XOR loop comparing character codes can mitigate the risk.
