...
## 2024-05-18 - Admin Token Timing Attack Prevention
**Vulnerability:** Timing attack vulnerability in admin token validation. The application used standard string equality (`!==`) to compare the provided `x-admin-token` with the server's `ADMIN_SECRET_TOKEN` in three separate routes (`admin.ts`, `ingest.ts`, `survey.ts`).
**Learning:** Standard string equality (`!==`) checks character-by-character and returns `false` as soon as a mismatch is found. This slight variation in response time based on the position of the mismatch can be measured by attackers to iteratively guess the correct secret token character-by-character.
**Prevention:** Always use a constant-time comparison algorithm (like a bitwise XOR loop) when comparing sensitive secrets or tokens to ensure the execution time is uniform regardless of where a mismatch occurs. Created a reusable `timingSafeEqual` function in `auth-utils.ts` for this purpose.
