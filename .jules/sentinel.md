...
## 2025-02-14 - Fix Timing Attack Vulnerability in Admin Token Authentication
**Vulnerability:** The admin authentication middleware in `worker/src/routes/admin.ts`, `worker/src/routes/ingest.ts`, and `worker/src/routes/survey.ts` was using a direct string comparison (`!==`) to compare the incoming `x-admin-token` with the server's `ADMIN_SECRET_TOKEN`. This exposes the endpoints to a timing attack, where an attacker could theoretically guess the token character by character by measuring the exact response time of the server.
**Learning:** Node.js native V8 string comparisons short-circuit when they find the first non-matching character. This means a comparison of "aXXX" against "bYYY" is faster than a comparison of "aXXX" against "aYYY". When checking authentication secrets or cryptographic hashes, this predictable time difference can be exploited.
**Prevention:** Always use a constant-time comparison function, such as a custom bitwise XOR loop implementation like `timingSafeEqual`, when verifying tokens, passwords, or secrets to prevent timing side-channel attacks.
