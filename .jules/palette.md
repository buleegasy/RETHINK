...
\n## 2026-09-03 - ARIA Live Announcements for Dynamic Status\n**Learning:** When using Framer Motion to swap dynamic status text (like connection or voice state) inside a shared container, screen readers often miss the updates without explicit ARIA live regions.\n**Action:** Add `aria-live="polite"` and `aria-atomic="true"` to the parent container wrapping the dynamic text function call to ensure smooth announcements for non-visual users.
