...
\n## 2025-02-23 - SVG Accessibility Redundancy Prevention\n**Learning:** When using screen readers, adding `aria-label` to a button is not always sufficient if the button contains an `<svg>` element. The screen reader might still try to read the SVG structure or announce 'graphic', leading to confusing or redundant announcements for pure icon buttons. This pattern is common in this app's icon-only buttons (like the Voice Input and Sidebar toggle).\n**Action:** Always explicitly add `aria-hidden="true"` to decorative `<svg>` elements inside interactive components (like `<button>` or `<a>`) that already provide their accessible name via `aria-label` or `aria-labelledby`.
