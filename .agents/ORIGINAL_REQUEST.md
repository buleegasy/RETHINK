...
## Follow-up — 2026-07-29T01:24:14Z

# Teamwork Project Prompt — Draft

全面审查心理大赛项目（前端 Web 与后端 Worker），确保之前“沙盘功能剥离”回滚操作后没有任何遗留的异常、悬空引用或被破坏的功能，并进行全库深度体检。
...
### R1. 全库静态类型分析与语法检查
执行整个项目的 TypeScript 静态检查和 Linter 规则检查，找出因为“沙盘功能剥离”导致的任何 `import` 引用错误、未定义变量以及未使用的废弃死代码。

### R2. 自动化测试健康度验证
执行项目内外现有的全部自动化单元测试套件（Web 端与 Worker 端），确保最近剥离沙盘逻辑的操作没有破坏现有的核心功能和意图路由。

### R3. 依赖完整性与构建检查
验证 `package.json` 的锁定文件与依赖树无冲突，确保项目在移除某些组件后依然能够成功执行 `build` 流程。

## Acceptance Criteria

### 静态分析与 Linting
- [ ] 运行 `tsc --noEmit`（前端和后端）不抛出任何关于 Sandplay 的缺失模块或类型错误。
- [ ] 运行项目的 Lint 检查（如 `eslint`）时，没有关于沙盘组件遗留的未使用 `import` 报错。

### 自动化测试
- [ ] Web 端运行 `npm run test:unit`（共约 62 个测试用例）全部 100% 成功通过，不出现模块导入崩溃。
- [ ] Worker 端运行 `npm run test:cov`（包含 FSM 和意图路由测试）全部成功通过。

### 最终报告
- [ ] 生成一个审查报告 Artifact，列出所有发现的潜在异常警告或确认全库健康的证明。
