...
## CBT 五阶段状态机 (Backend Only)

这五步流程现已转为纯后台静默运行，保证前台 UI 体验的纯粹与自然。通过 SSE 响应元数据携带至前端，仅在展开消息的“后台意图”中可见。
...
## 设计语言 (Gemini 1:1 Replica)

> **核心原则**：完全移除早期任何带有 “Nothing 风格”（如点阵字、生硬直角、单色调）的设计元素，全量替换为 **最新版 Gemini 的超流体、强光效玻璃拟物化视觉**。

- **动态空状态 (GeminiWelcome)**：进入页面时，中心的星火 Logo 会伴随着 Framer Motion 的 `spring` 物理弹簧效果进行“Duang~”的弹出展示，背景辅以缓缓脉冲的蓝紫色光晕。
- **思考光效 (Thinking Glow)**：当 AI 处于流式生成（思考）状态时，底部输入框和 AI 头像会激活 `.gemini-thinking-glow`（通过多重渐变背景 `background-size: 200%` 实现流动的彩虹呼吸光效）。
- **极简对话流**：
  - **AI 气泡**：不再使用生硬的卡片边界。AI 的回复作为纯文本直接渲染在背景图层上，辅以极度舒适的排版间距（Prose）。
  - **User 气泡**：使用超大圆角（`32px`）的浅色胶囊背景，贴近 Gemini 官方视觉。
- **底部输入区域**：采用悬浮胶囊式设计（Floating Pill），去除实线边框，采用高斯模糊阴影和细微的玻璃拟物化背景，在活跃时四周边缘流转光效。
- **主字体**：彻底统一为 `Google Sans` 或 `Inter`，完全剔除代码感过强的等宽字体在正文中的滥用，保证丝滑的阅读体验。
