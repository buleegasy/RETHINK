            ...
            {!isUser && tc && (
              <div className="mt-1 w-full select-none animate-slide-up ms-4">
                ...
                >
                  ...
                  {showTechChain ? <ChevronUp className="w-3 h-3" aria-hidden="true" /> : <ChevronDown className="w-3 h-3" aria-hidden="true" />}
                </button>
                                ...
                                {tc.ragSnippets.map((snippet, i) => {
                                  ...
                                  return (
                                    <div key={i} className="text-[10px] space-y-0.5">
                                      ...
                                      {isExpanded && (
                                        <div
                                          id={`rag-snippet-${message.id}-${i}`}
                                          className="bg-surface/60 p-1.5 rounded border border-outline-variant/20 text-[10px] text-on-surface-variant whitespace-pre-wrap leading-normal font-sans"
                                        >
                                          {snippet}
                                        </div>
                                      )}
